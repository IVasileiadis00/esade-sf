...
Metrics calculated:
  ...
  - Beta vs benchmark (EXW1.DE = STOXX 600 proxy)
...
RISK_FREE_RATE = 0.0   # assumption: 0% risk-free rate (document in audit log)
...
def sharpe(series):
    """Sharpe ratio: (1Y return - risk_free_rate) / annualised_volatility."""
    ret = total_return(series, 1)
    vol = annualised_vol(series)
...
for ticker in prices.columns:
    ...
    if len(s) < TRADING_DAYS:
        print(f"  FLAGGED: {ticker} — only {len(s)} rows (< {TRADING_DAYS}). Metrics will be partial.")
